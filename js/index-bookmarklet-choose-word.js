...
`;
    ...
    defHelp.innerText = "Hover over a word to highlight, then click/tap to show definition tooltip. Where a word has punctuation, a [list] has been created to allow other choices.";
    defSwitch.appendChild(defHelp);
    let spans;
    let p = document.getElementsByTagName("p");
    let originalP = [];
    /*if punctuation, break into strings containing unique punct forms*/
    // const checkPunct = (word) => {
    //     console.log(word + "\n");
    //     let newWord = word;
    //     const punctuationMarks = ["'", ".", "-", ",", '"'];
    //     [...punctuationMarks].forEach(mark => {
    //         console.log(mark);
    //         let punct = word.indexOf(mark)
    //         console.log(punct);
    //         if (punct !== -1) {
    //             if (mark === '-') {
    //                 newWord += " " + word.split(mark).join(' ');
    //             } else
    //                 newWord += " " + word.split(mark).join('');
    //         }
    //     });
    ...
    const addSpans = (paragraph) => {
        ...
        words.forEach(word => {
            // if (word.includes("-")) {
            //     hyphen = word.split("-");
            //     hyphen.forEach(noHyphen => {
            //         newParagraph += `<span>${noHyphen}</span> `;
            //     })
            // } else {
